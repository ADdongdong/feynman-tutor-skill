...
> 版本：V1.0 | 日期：2026-07-24 | 最近更新：2026-08-05（v1.1.7：新增节点7.5 模块级大作业，代码学习闭环）
...
GOAL_SELECT     → 节点2：按学科阶梯的本族化目标选择（3~5 级，无摸底题）
...
PRACTICE        → 节点7：做中学（条件触发）
  ...
  │    └─ 否 → 进入 PRACTICE
  └─ PRACTICE 完成 → 节点7.5（代码类学习且模块标注大作业时）
  （深度级数由学科声明，可配置 3~5；难度按相对位置：中间级=轻量版，最高级=重量版）
MODULE_ASSIGN    → 节点7.5：模块级大作业（代码类学习，条件触发；v1.1.7 新增）
  ├─ 前置：节点3 已标注该模块「大作业：有」；在 feynman/lab-<模块> 分支上执行
  ├─ 形式按场景四态：学习副本/自建学习 repo → 重量版；工作区 → 轻量版；片段 → 重量自由
  ├─ 模板：目标 → 改动 → 预测 → 验证 → 结论
  ├─ 降级：跑不起来 → 静态分析预测 / 独立模块实验 / 口述预测
  └─ 作业记录双轨归档（分支 NOTES.md + Obsidian 作业记录子块）→ MODULE_DONE
DIAGNOSE        → 节点6：诊断根因
